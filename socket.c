...
void client_command(getCommand *getcmd)
{
    ...
    while (true)                // 무한 루프를 실행한다.
    {
        ...
        // getcmd 구조체의 command 멤버 변수에 저장된
        // 문자열을 json 형태로 파싱하여 rootObj 멤버 변수에 저장한다.
        getcmd->rootObj = json_tokener_parse(getcmd->command);  
        pthread_mutex_unlock(&g_mutex);  // g_mutex 뮤텍스를 언락한다.
    }
}
...
{
    pthread_mutex_lock(&g_mutex);                   
    write(getcmd->connection, msg, strlen(msg)); // 클라이언트에게 응답을 보낸다.
    pthread_mutex_unlock(&g_mutex);                  
    getcmd->command = NULL;  // getcmd 구조체의 command 멤버를 NULL로 초기화한다.
    getcmd->rootObj = NULL;  // getcmd 구조체의 rootObj 멤버를 NULL로 초기화한다.
}
...
// 문자열 비교 함수
int strcmps(char *msg1, char *msg2) {
    // 두 문자열이 모두 NULL이 아닐 경우 strcmp 함수를 호출하여 비교한 결과를 반환
    if (msg1 != '\0' && msg2 != '\0') return strcmp(msg1, msg2);
    // 그렇지 않은 경우 -1 반환
    else
        return -1;
}

// JSON 객체에서 특정 명령어가 있는지 확인하는 함수
bool checkCommand(json_object *data, const char *command) {
    // JSON 객체에서 해당 명령어를 가져옴
    json_object *commandObj = json_object_object_get(data, command);
    // 가져온 명령어와 입력한 명령어를 비교하여 같으면 true 반환
    if (strcmps(json_object_get_string(commandObj), command) == 0) {
        return true;
    }
    // 그렇지 않은 경우 false 반환
    return false;
